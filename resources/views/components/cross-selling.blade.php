...
@if ($products->isNotEmpty())
    <div class="mt-3 mb-3">
        <h2 class="section-title after-line mb-3">{{ $title }}</h2>
        @if ($slider)
            <div class="owl-carousel owl-cross-selling owl-theme">
                @foreach ($products as $product)
                    <div class="item">
                        @if ($type == 'course')
                            @include('web.default.includes.webinar.grid-card', ['webinar' => $product])
                        @elseif($type == 'blog')
                            @include('web.default.blog.grid-list', ['post' => $product])
                        @elseif($type == 'product')
        ...
        @else
            <div class="row">
                @foreach ($products as $product)
                    <div class="{{ $display == 2 ? 'col-12 col-lg-12 mt-20' : 'col-12 col-lg-6 mt-20' }}">
                        @if ($type == 'course')
                            @include('web.default.includes.webinar.grid-card', ['webinar' => $product])
                        @elseif($type == 'blog')
                            @include('web.default.blog.grid-list', ['post' => $product])
