    ...
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item">Product Discounts</div>
            </div>
        </div>
                                    ...
                                    <select name="product_id" id="product_id" class="form-control @error('product_id') is-invalid @enderror">
                                        ...
                                        @foreach($products as $product)
                                            <option value="{{ $product['id'] }}" 
                                                ...
                                                {{ isset($productDiscounts) && $productDiscounts->product_id == $product['id'] ? 'selected' : '' }}>
                                                {{ $product['title'] }}
                                            </option>
                                        @endforeach
                                    </select>
                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-striped font-14 text-center">
                                    ...
                                    @foreach($specialOffersProductDiscounts as $specialOffer)
                                        <tr>
                                            <td>{{ $specialOffer->name }}</td>
                                            {{-- ===== product name ===== --}}
                                            <td class="text-left">
                                                <span class="d-block font-14">{{ $specialOffer->product->title }}</span>
                                            </td>
                                            ...
                                            <td>
                                                @can('admin_product_discount_edit')
                                                    <a href="{{ getAdminPanelUrl() }}/financial/special_offer_product_discounts/{{ $specialOffer->id }}/edit" class="btn-transparent text-primary btn-sm" data-toggle="tooltip" data-placement="top" title="{{ trans('admin/main.edit') }}">
                                                        <i class="fa fa-edit"></i>
                                                    </a>
                                                @endcan
                                                @can('admin_product_discount_delete')
                                                    @include('admin.includes.delete_button',['url' => getAdminPanelUrl().'/financial/special_offer_product_discounts/'. $specialOffer->id.'/delete','btnClass' => ''])
                                                @endcan
                                            </td>
                                        </tr>
                                    @endforeach
                        ...
                        <div class="card-footer text-center">
                            {{ $specialOffersProductDiscounts->appends(request()->input())->links() }}
                        </div>
                    </div>
                </div>
            </div>
